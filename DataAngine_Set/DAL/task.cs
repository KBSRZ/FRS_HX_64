...
namespace DataAgine_Set.DAL
{
	...
	public partial class task
	{
		...
		public int GetMaxId()
		{
		return DbHelperMySQL.GetMaxID("id", "task"); 
		}
		...
		public bool Add(DataAgine_Set.Model.task model)
		{
			...
			strSql.Append("databaseid,deviceid)");
		...
		public DataAgine_Set.Model.task GetModel(int id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,databaseid,deviceid from task ");
			strSql.Append(" where id=@id");
			MySqlParameter[] parameters = {
					new MySqlParameter("@id", MySqlDbType.Int32)
			};
			...
			DataAgine_Set.Model.task model=new DataAgine_Set.Model.task();
		...
		/// 得到一个对象实体
		/// </summary>
		public DataAgine_Set.Model.task DataRowToModel(DataRow row)
		{
			DataAgine_Set.Model.task model=new DataAgine_Set.Model.task();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select id,databaseid,deviceid ");
