CREATE DATABASE `frdb`;

CREATE USER 'frba'@'localhost' IDENTIFIED BY '12345679'; 

GRANT ALL PRIVILEGES ON `frdb`.* TO 'frba'@'%' ;
GRANT ALL PRIVILEGES ON `frdb`.* TO 'frba'@'localhost' ;
FLUSH PRIVILEGES;

USE `frdb`;

create table `frdb`.`user`
(
...
create table `frdb`.`hitrecord_detail`
(
	`id` int(11) AUTO_INCREMENT,
...
FROM 
(`frdb`.`hitrecord_detail` as detail
left join `frdb`.`hitrecord` as hit on detail.hit_record_id=hit.id) 
left join `frdb`.`user` as usr on detail.user_id = usr.id;